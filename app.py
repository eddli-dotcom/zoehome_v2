...
<style>
    ...
    /* 🚨 針對手機深色模式的強制修復 🚨 */
    input, textarea { background-color: #ffffff !important; color: #333333 !important; }
    ...
    li[role="option"] { color: #333333 !important; background-color: #ffffff !important; }
    li[role="option"]:hover, li[role="option"][aria-selected="true"] { background-color: #fff5e6 !important; color: #B8860B !important; }
    div[data-testid="stToast"] { background-color: #ffffff !important; border: 2px solid #FFD700 !important; box-shadow: 0 4px 15px rgba(0,0,0,0.15) !important; }
    div[data-testid="stToast"] * { color: #333333 !important; }
    div[data-testid="stPopoverBody"] { background-color: #ffffff !important; border: 2px solid #FFD700 !important; }
    div[data-testid="stPopoverBody"] * { color: #333333 !important; }

    /* 🌈 裝備稀有度顏色標籤 🌈 */
    .r-leg { background: linear-gradient(135deg, #ff8c00 0%, #ff4500 100%); color: white !important; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; display: inline-block; box-shadow: 0 2px 4px rgba(255,140,0,0.3);}
    .r-epi { background: linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%); color: white !important; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; display: inline-block; box-shadow: 0 2px 4px rgba(155,89,182,0.3);}
    .r-rar { background: linear-gradient(135deg, #3498db 0%, #2980b9 100%); color: white !important; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; display: inline-block; box-shadow: 0 2px 4px rgba(52,152,219,0.3);}
    .r-com { background: linear-gradient(135deg, #2ecc71 0%, #27ae60 100%); color: white !important; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; display: inline-block; box-shadow: 0 2px 4px rgba(46,204,113,0.3);}
    .gear-desc { font-size: 0.85rem; color: #7f8c8d !important; margin-top: 4px; display: block;}
    .gear-item-box { border-bottom: 1px dashed #eee; padding: 8px 0; }
...
STORE_ITEMS = {"免做家事金牌 🛡️": 40, "幫我整杯茶/水果 ☕": 20, "今晚電視控制權 📺": 30, "按摩 10 分鐘 💆": 30, "指定對方做一件家事 👉": 50}

ZOE_MESSAGES = [
    "Zoe 滿心期待搖著尾巴！🐾 快去做家務啦！", "屋企乾淨嘅話，Zoe會超開心！✨",
    "汪汪！記得檢查裝備嘅耐久度，去鐵匠鋪可以修復！🔨", "累積滿 200 金幣，Zoe 就會降臨並帶來超大禮盒喔！🌟",
    "今日邊個係家務大師呢？Zoe 望實你地！👀", "連續做同一個任務會有連擊加成，越做越賺喔！🔥",
    "每日有d任務有 1.5 倍加成，千祈唔好錯過啦！📈", "望望望，仲唔快d開工！💖",
    "稀有裝備『黃金掃把』係可以令金幣翻倍嘅神器喔！🧹", "打敗突發嘅世界 BOSS 可以獲得大量金幣！🐉",
    "豬頭骨任務會自動累積賞金，快去做賞金獵人啦！🎯", "多餘嘅普通裝備可以去鐵匠鋪合成稀有神器！🧪"
]

GIFT_MSGS = [
    "為你獻上我的傳家寶！ 💎", "見你咁辛苦，獎你嘅！別太愛我 😏",
    "依個係我千辛萬苦抽返來的，好好珍惜啊！ ⚔️", "你明啦 ❤️",
    "拿去吧！這就是強者的餘裕 😎", "這個超適合你，快戴上給大爺/老娘看看！ 😘",
    "收下吧 (笑) 😎"
]
...
BOSS_LIST = [
    ...
    {"name": "🦠 擊退季節性致敏原", "chores": ["🧹 拖/掃地", "👕 洗衣服", "🛁 洗浴缸"], "reward": 60},
...
def get_rarity_html(rarity_str):
    mapping = {
        "Legendary": ("r-leg", "傳說"),
        "Epic": ("r-epi", "史詩"),
        "Rare": ("r-rar", "稀有"),
    ...
    c_class, c_name = mapping.get(rarity_str, ("r-com", "普通"))
    return f"<span class='{c_class}'>{c_name}</span>"

def get_rarity_text(rarity_str):
    mapping = {"Legendary": "傳說", "Epic": "史詩", "Rare": "稀有", "Common": "普通"}
    return mapping.get(rarity_str, "普通")
...
def check_achievements(person):
    ...
    # 🚽 廁所系列
    if s["chores"].get("🚽 洗厠所", 0) >= 1: award("御用洗廁所專員 🚽")
    if s["chores"].get("🚽 洗厠所", 0) >= 3: award("廁所守護者 🚽🚽")
    if s["chores"].get("🚽 洗厠所", 0) >= 10: award("廁所守護神 🚽🚽🚽")
    if s["chores"].get("🚽 洗厠所", 0) >= 20: award("人廁一體 🚽🚽🚽🚽")
    if s["chores"].get("🚽 洗厠所", 0) >= 50: award("馬桶之王 🚽🚽🚽🚽🚽")

    # 💧 玩水系列
    if sum(v for k, v in s["chores"].items() if "洗" in k or "水" in k) >= 10: award("喜歡玩水 💧")
    if sum(v for k, v in s["chores"].items() if "洗" in k or "水" in k) >= 30: award("玩水大師 💧💧")
    if sum(v for k, v in s["chores"].items() if "洗" in k or "水" in k) >= 50: award("五行缺水 💧💧💧")
    if sum(v for k, v in s["chores"].items() if "洗" in k or "水" in k) >= 100: award("水魚降臨 💧💧💧💧")
    if sum(v for k, v in s["chores"].items() if "洗" in k or "水" in k) >= 500: award("我愛玩水 💧💧💧💧💧")

    # 🗑️ 垃圾系列
    if sum(v for k, v in s["chores"].items() if "垃圾" in k) >= 1: award("垃圾婆/佬 🗑️")
    if sum(v for k, v in s["chores"].items() if "垃圾" in k) >= 5: award("高級垃圾婆/佬 🗑️🗑️")
    if sum(v for k, v in s["chores"].items() if "垃圾" in k) >= 20: award("拾荒大王 🗑️🗑️🗑️")
    if sum(v for k, v in s["chores"].items() if "垃圾" in k) >= 50: award("本命拾荒 🗑️🗑️🗑️🗑️")
    if sum(v for k, v in s["chores"].items() if "垃圾" in k) >= 100: award("垃圾本人 🗑️🗑️🗑️🗑️🗑️")

    # 🐾 汪星人系列
    if sum(v for k, v in s["chores"].items() if "狗" in k) >= 1: award("Zoe嘅專屬奴才 🐾")
    if sum(v for k, v in s["chores"].items() if "狗" in k) >= 10: award("汪星人摯友 🐾")
    if sum(v for k, v in s["chores"].items() if "狗" in k) >= 30: award("金牌伴讀犬 🐾🐾")
    if sum(v for k, v in s["chores"].items() if "狗" in k) >= 100: award("狗語者 🐾🐾🐾")
    if sum(v for k, v in s["chores"].items() if "狗" in k) >= 200: award("狗餅提款機 🐾🐾🐾🐾")

    # 👕 洗衣系列
    if sum(v for k, v in s["chores"].items() if "衣服" in k) >= 1: award("阿四 👕")
    if sum(v for k, v in s["chores"].items() if "衣服" in k) >= 10: award("洗衣大師 👕👕")
    if sum(v for k, v in s["chores"].items() if "衣服" in k) >= 30: award("人肉乾衣機 👕👕👕")
    if sum(v for k, v in s["chores"].items() if "衣服" in k) >= 100: award("輕鬆家務愛好者 👕👕👕👕")
    if sum(v for k, v in s["chores"].items() if "衣服" in k) >= 200: award("冇你全家裸體 👕👕👕👕👕")

    # 🌪️ 總家務次數
    if s["total_chores"] >= 1: award("終於肯郁 🌪️")
    if s["total_chores"] >= 20: award("家務新手 🌪️")
    if s["total_chores"] >= 50: award("家務大師 🌪️🌪️")
    if s["total_chores"] >= 100: award("家務聖手 🌪️🌪️🌪️")
    if s["total_chores"] >= 200: award("家務天尊 🌪️🌪️🌪️🌪️")
    if s["total_chores"] >= 500: award("天生勞碌命 🌪️🌪️🌪️🌪️🌪️")
    if s["total_chores"] >= 1000: award("賓妹/賓仔 🌪️🌪️🌪️🌪️🌪️")

    # ⚡ 連擊系列
    if s["max_streak"] >= 3: award("連擊大師 ⚡")
    if s["max_streak"] >= 5: award("連擊宗師 ⚡⚡")
    if s["max_streak"] >= 10: award("連擊大佬 ⚡⚡⚡")
    if s["max_streak"] >= 20: award("連擊王 ⚡⚡⚡⚡")
    if s["max_streak"] >= 50: award("自閉之王 ⚡⚡⚡⚡⚡")

    # 🎯 賞金系列
    if s["max_bounty"] >= 5: award("賞金獵人 🎯")
    if s["max_bounty"] >= 10: award("賞金宗師 🎯🎯")
    if s["max_bounty"] >= 20: award("賞金巨子 🎯🎯🎯")
    if s["max_bounty"] >= 50: award("專執二攤 🎯🎯🎯🎯")
    ...
    # 🎰 盲盒系列
    if s["gacha_pulls"] >= 1: award("試伏器 🎰")
    if s["gacha_pulls"] >= 5: award("盲盒初心者 🎰")
    if s["gacha_pulls"] >= 10: award("盲盒成癮者 🎰🎰")
    if s["gacha_pulls"] >= 50: award("盲盒大王 🎰🎰🎰")
    if s["gacha_pulls"] >= 100: award("賭徒境界 🎰🎰🎰🎰")
    if s["gacha_pulls"] >= 200: award("傾家蕩產 🎰🎰🎰🎰🎰")
    if s["gacha_pulls"] >= 500: award("非洲大酋長 🎰🎰🎰🎰🎰🎰")

    # 💥 裝備破壞系列
    if s["gear_broken"] >= 1: award("裝備破壞王 💥")
    if s["gear_broken"] >= 5: award("搞屎棍 💥💥")
    if s["gear_broken"] >= 10: award("粉碎者 💥💥💥")
    if s["gear_broken"] >= 50: award("破壞神 💥💥💥💥")

    # 🎯 Bingo系列
    if s["bingos"] >= 1: award("完美Bingo 🎯")
    if s["bingos"] >= 5: award("Bingo殺手 🎯🎯")
    if s["bingos"] >= 10: award("BINGO之王 🎯🎯🎯")

    # 🙏 Zoe系列
    if s["zoe_100"] >= 1: award("Zoe的信徒 🙏")
    if s["zoe_100"] >= 5: award("Zoe照顧員 🙏🙏")
    if s["zoe_100"] >= 25: award("Zoe最愛 🙏🙏🙏")
    if s["zoe_100"] >= 50: award("Zoe主人 🙏🙏🙏🙏")
    if s["zoe_100"] >= 100: award("Zoe首席主人 🙏🙏🙏🙏🙏")

    # 💸 課金/商店系列
    if s["store_bought"] >= 1: award("散財童子 💸")
    if s["store_bought"] >= 5: award("課金細佬 💸")
    if s["store_bought"] >= 10: award("課金大佬 💸💸")
    if s["store_bought"] >= 30: award("大金主 💸💸💸")
    if s["store_bought"] >= 100: award("終極大水魚 💸💸💸💸")

    # 👑 等級系列
    if u["level"] >= 10: award("滿級大佬 👑")
    if u["level"] >= 25: award("轉生尊者 👑👑")
    if u["level"] >= 50: award("一家之二主 👑👑👑")
    if u["level"] >= 100: award("一家之主 👑👑👑👑")

    # 💰 財富系列
    if s["total_earned"] >= 10: award("乞衣 💰")
    if s["total_earned"] >= 50: award("窮過見鬼 💰💰")
    if s["total_earned"] >= 100: award("餐搵餐食 💰💰💰")
    if s["total_earned"] >= 200: award("養得起Zoe 💰💰💰💰")
    if s["total_earned"] >= 500: award("漸入佳境 💰💰💰💰💰")
    if s["total_earned"] >= 1000: award("財富自由 💰💰💰💰💰💰")
    if s["total_earned"] >= 5000: award("好能有錢 💰💰💰💰💰💰💰")
    if s["total_earned"] >= 10000: award("守財奴 💰💰💰💰💰💰💰💰")

    if sum(v for k, v in s["chores"].items() if "地" in k) >= 1: award("人肉掃地機 🧹")
    if sum(v for k, v in s["chores"].items() if "地" in k) >= 10: award("地板反光師 🧹✨")
    if sum(v for k, v in s["chores"].items() if "地" in k) >= 30: award("隱世掃地僧 🧘‍♂️🧹✨")
    if sum(v for k, v in s["chores"].items() if "地" in k) >= 100: award("地平線摩擦王 🏎️🧘‍♂️🧹✨")
    
    if s["chores"].get("🏠 收拾家居", 0) >= 1: award("亂室佳人 🏠")
    if s["chores"].get("🏠 收拾家居", 0) >= 10: award("眼不見為乾淨 🙈🏠")
    if s["chores"].get("🏠 收拾家居", 0) >= 30: award("斷捨離教主 🗑️🙈🏠")
    if s["chores"].get("🏠 收拾家居", 0) >= 100: award("空間摺疊大師 🌌🗑️🙈🏠")

    if sum(v for k, v in s["chores"].items() if "飯" in k or "菜" in k or "煲水" in k) >= 1: award("暗黑料理界 🍳")
    if sum(v for k, v in s["chores"].items() if "飯" in k or "菜" in k or "煲水" in k) >= 10: award("無飯家庭救星 🥡🍳")
    if sum(v for k, v in s["chores"].items() if "飯" in k or "菜" in k or "煲水" in k) >= 50: award("頹飯供應商 🍱🥡🍳")
    if sum(v for k, v in s["chores"].items() if "飯" in k or "菜" in k or "煲水" in k) >= 70: award("米芝蓮痴線星 🌟🍱🥡🍳")
    if sum(v for k, v in s["chores"].items() if "飯" in k or "菜" in k or "煲水" in k) >= 150: award("米芝蓮三星 🌟🌟🌟")
    if sum(v for k, v in s["chores"].items() if "飯" in k or "菜" in k or "煲水" in k) >= 300: award("米芝蓮本人 🌟🌟🌟🌟")

    if s.get("gear_repaired", 0) >= 1: award("補鑊達人 🔧")
    if s.get("gear_repaired", 0) >= 10: award("破銅爛鐵收買佬 🛠️🔧")
    if s.get("gear_merged", 0) >= 1: award("神棍煉金術士 🧪🛠️🔧")
    if s.get("gear_merged", 0) >= 10: award("鋼之煉金水魚 🤖🧪🛠️🔧")

    if s.get("gifts_sent", 0) >= 1: award("借花敬佛 🌸")
    if s.get("gifts_sent", 0) >= 5: award("廿四孝伴侶 💑🌸")
    if s.get("gifts_sent", 0) >= 20: award("首席大兵 🪖💑🌸")
    if s.get("gifts_sent", 0) >= 50: award("舔狗最高境界 👅🪖💑🌸")
...
def handle_dynamic_completion(person, chore, diff_level):
    ...
    if is_hot_chore:
        ...
        st.toast("🔥 黃金任務加成！金幣與 EXP 1.5 倍！", icon="📈")
...
with tab1:
    ...
    for chore, info in app_data["chores_info"].items():
        ...
        hot_html = f"<span class='badge-base bg-blue'>🌟 黃金任務獎勵 1.5x</span>" if is_hot else ""
...
with tab2:
    for person, icon in [("老公", "👨"), ("老婆", "👩")]:
        ...
        gear = next((g for g in u["gear_inventory"] if g["uid"] == u["equipped_gear"]), None)
        if gear:
            gd = GEAR_DATA[gear['name']]
            r_html = get_rarity_html(gd['rarity'])
            gear_display = f"{r_html} <span style='font-weight:bold; color:#333;'>{gear['name']}</span> (耐久: {gear['durability']}/{gear['max_durability']})<br><span class='gear-desc'>📖 {gd['desc']}</span>"
        else:
            gear_display = "<span style='color:#7f8c8d; font-weight:bold;'>無裝備</span>"
        ...
        st.markdown(f"""<div class='profile-card'>
            ...
            <div style='background:#fdfbf7; padding:10px; border-radius:8px; border-left:4px solid #B8860B; margin-top:10px;'>
                <p style='margin:0 0 4px 0; font-size:0.85rem; font-weight:bold; color:#555;'>🗡️ 目前裝備：</p>
                {gear_display}
            </div>
        </div>""", unsafe_allow_html=True)
        ...
        # 🎒 互動式背包系統 (點擊物品管理)
        with st.expander(f"🎒 {person} 的背包 (點擊物品管理)"):
            st.markdown("<h5 style='color:#B8860B; margin:0;'>🛡️ 裝備庫</h5>", unsafe_allow_html=True)
            if not u["gear_inventory"]: 
                st.write("空空如也")
            else:
                g_cols = st.columns(2)
                for i, g in enumerate(u["gear_inventory"]):
                    gd = GEAR_DATA[g['name']]
                    r_html = get_rarity_html(gd['rarity'])
                    with g_cols[i % 2].popover(f"{get_emoji(g['name'])} {g['name']}", use_container_width=True):
                        st.markdown(f"{r_html} <span style='font-size:1.1rem; font-weight:bold; color:#333;'>{g['name']}</span>", unsafe_allow_html=True)
                        st.markdown(f"<span class='gear-desc'>📖 <b>功能:</b> {gd['desc']}</span>", unsafe_allow_html=True)
                        st.write(f"🛡️ **耐久度:** {g['durability']} / {g['max_durability']}")
                        if u["equipped_gear"] == g["uid"]:
                            st.info("✅ 裝備中")
                        if st.button("🗑️ 丟棄裝備", key=f"del_g_{person}_{g['uid']}"):
                            if u["equipped_gear"] == g["uid"]: u["equipped_gear"] = None
                            u["gear_inventory"] = [item for item in u["gear_inventory"] if item["uid"] != g["uid"]]
                            save_data(app_data); st.rerun()
                            
            st.markdown("<h5 style='color:#B8860B; margin-top:15px; margin-bottom:5px;'>💊 消耗品</h5>", unsafe_allow_html=True)
            if not u["inventory"]: 
                st.write("空空如也")
            else:
                c_cols = st.columns(2)
                for i, item in enumerate(u["inventory"]):
                    with c_cols[i % 2].popover(f"▪️ {item}", use_container_width=True):
                        st.markdown(f"<span style='font-size:1.1rem; font-weight:bold; color:#333;'>{item}</span>", unsafe_allow_html=True)
                        st.write("📖 單次消耗品，使用後消失。")
                        if st.button("✨ 使用", key=f"use_{person}_{i}_{item}"):
                            u["inventory"].pop(i)
                            app_data["history"].insert(0, {"time": datetime.now().strftime("%m/%d %H:%M"), "person": person, "chore": f"✨ 使用了：{item}", "coins": 0})
                            save_data(app_data); st.rerun()
                        if st.button("🗑️ 丟棄", key=f"del_c_{person}_{i}_{item}"):
                            u["inventory"].pop(i)
                            save_data(app_data); st.rerun()
        ...
        with st.expander(f"🛠️ {person} 的設定 (換稱號/職業/裝備)"):
            ...
            def format_gear_dropdown(g): 
                if g is None: return "無裝備"
                r_text = get_rarity_text(GEAR_DATA[g['name']]['rarity'])
                return f"[{r_text}] {g['name']} (耐久: {g['durability']}/{g['max_durability']})"
                
            curr_idx = next((i for i, g in enumerate(gear_opts) if g and g["uid"] == u["equipped_gear"]), 0)
            new_gear = st.selectbox("穿戴裝備", gear_opts, index=curr_idx, format_func=format_gear_dropdown, key=f"gr_{person}")
        ...
        if u["inventory"] or u["gear_inventory"]:
            with st.expander(f"🎁 送禮物給伴侶"):
                ...
                if gift_type == "消耗品" and u["inventory"]:
                    ...
                    gift_item = st.selectbox("選擇要送的裝備", u["gear_inventory"], format_func=format_gear_dropdown, key=f"ggear_{person}")
                else:
                    gift_item = None; st.info("此分類下沒有物品可以贈送喔！")
                ...
                if gift_item and st.button(f"💌 寄送給 {partner}", key=f"send_{person}", use_container_width=True):
                    app_data["users"][person]["stats"]["gifts_sent"] += 1
                    if gift_type == "消耗品":
...
with tab3:
    ...
    def pull_gacha(person):
        if app_data[person] >= 15:
            ...
            if random.random() < 0.40:
                prize = random.choice(["免做家事金牌 🛡️", "跑腿券 🏃", "按摩券 💆", "謝謝惠顧 (Zoe 的飛吻 💋)"])
    ...
    with st.expander("🔍 查看盲盒獎池 (裝備圖鑑)"):
        st.markdown("<p style='font-size:0.85rem; color:#666;'>查看所有可能抽出的稀有裝備與功能。</p>", unsafe_allow_html=True)
        for rarity in ["Legendary", "Epic", "Rare", "Common"]:
            st.markdown(f"<div style='margin-top:10px;'>{get_rarity_html(rarity)}</div>", unsafe_allow_html=True)
            for name, d in GEAR_DATA.items():
                if d["rarity"] == rarity:
                    st.markdown(f"<div class='gear-item-box'><span style='font-weight:bold; color:#333;'>{name}</span><br><span class='gear-desc'>📖 {d['desc']}</span></div>", unsafe_allow_html=True)
    ...
    for item, base_price in STORE_ITEMS.items():
        ...
        st.markdown(f"<div style='background: white; padding: 10px; border-radius: 8px; border: 1px solid #eee; margin-bottom: 5px;'>"
                    f"<span style='font-weight:bold; color:#333;'>{item}</span> (💰 {price}) {flash_badge}</div>", unsafe_allow_html=True)
...
with tab4:
    ...
    if repair_inv:
        def format_rep_gear(g):
            r_text = get_rarity_text(GEAR_DATA[g['name']]['rarity'])
            return f"[{r_text}] {g['name']} (目前: {g['durability']}/{g['max_durability']})"
            
        gear_to_repair = st.selectbox("選擇要修復的裝備", repair_inv, format_func=format_rep_gear)
        if st.button("🔨 進行修復 (-20🪙)", use_container_width=True):
            if app_data[repair_person] >= 20:
                ...
                app_data["users"][repair_person]["stats"]["gear_repaired"] += 1
                ...
                save_data(app_data); 
                check_achievements(repair_person)
                st.success("修復成功！裝備煥然一新！"); time.sleep(1); st.rerun()
    ...
    st.markdown("#### 🧪 煉金合成爐")
    st.markdown("<p style='font-size:0.9rem; color:#666;'>選擇 3 件<span class='r-com'>普通</span>裝備，必定煉成 1 件<span class='r-rar'>稀有</span>裝備！</p>", unsafe_allow_html=True)
    ...
    if len(common_gears) >= 3:
        ...
        if len(selected_to_merge) == 3:
            if st.button("🔥 立即合成", use_container_width=True):
                ...
                app_data["users"][merge_person]["stats"]["gear_merged"] += 1
                
                app_data["history"].insert(0, {"time": datetime.now().strftime("%m/%d %H:%M"), "person": merge_person, "chore": f"🧪 煉金合成，獲得稀有裝備：{new_rare}", "coins": 0})
                save_data(app_data); 
                check_achievements(merge_person)
                st.balloons(); st.success(f"合成成功！獲得稀有神器：【{new_rare}】！"); time.sleep(2); st.rerun()
...
with tab6:
    ...
    if st.button("🚨 立即發佈世界 BOSS！", use_container_width=True):
        if not custom_boss_name or not custom_boss_chores: st.error("請填寫 BOSS 名稱並至少選擇一個任務！")
        elif app_data["world_boss"]["active"]: st.error("目前已經有活躍的世界 BOSS，請先擊敗牠！")
        else:
